...
const octokit = new Octokit({
	auth: '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'
})
...
/**	var owner=['Homebrew','dotnet','zephyrproject-rtos','qgis']
	var repo=['homebrew-cask','roslyn','zephyr','QGIS']
**/
	for (var i=0;i<1;i++){
		var Alldata=[]
		var l=1;
//		let data=[{url: "000"}]
		...
		let {data}=await octokit.request('GET /repos/{owner}/{repo}/issues/comments',{
			owner: owner[i],
			repo: repo[i],
			state: "all",
			per_page: 100,
...
//		writerStream.write(Alldata,'UTF-8');
		while(data!=null){
			let {data}=await octokit.request('GET /repos/{owner}/{repo}/issues/comments',{
				owner: owner[i],
				repo: repo[i],
...
//			console.log(data);
			...
			while(data[k]!=null){
				delete data[k].url;
				delete data[k].html_url;
				delete data[k].id;
				delete data[k].node_id;
				delete data[k].user.id;
				delete data[k].user.node_id;
				delete data[k].user.avatar_url;
				delete data[k].user.gravatar_id;
				delete data[k].user.followers_url;
				delete data[k].user.following_url;
				delete data[k].user.gists_url;
				delete data[k].user.starred_url;
				delete data[k].user.subscriptions_url;
				delete data[k].user.organizations_url;
				delete data[k].user.repos_url;
				delete data[k].user.events_url;
				delete data[k].user.received_enents_url;
				delete data[k].user.site_admin;
				delete data[k].reactions;
				delete data[k].performed_via_github_app;
				k++;
			}
			l++;
//			console.log(data)
